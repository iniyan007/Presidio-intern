    ...
    public PackageMappingProfile()
    {
        ...
        CreateMap<Package, PackageDetailResponse>()
            ...
            .ConstructUsing((src, ctx) => new PackageDetailResponse(
                ...
                src.AvgRating,
                src.TotalReviews,
                src.Reviews != null && src.Reviews.Any(r => r.AccommodationRating.HasValue) ? (decimal?)Math.Round(src.Reviews.Where(r => r.AccommodationRating.HasValue).Average(r => r.AccommodationRating!.Value), 1) : null,
                src.Reviews != null && src.Reviews.Any(r => r.TransportRating.HasValue) ? (decimal?)Math.Round(src.Reviews.Where(r => r.TransportRating.HasValue).Average(r => r.TransportRating!.Value), 1) : null,
                src.Reviews != null && src.Reviews.Any(r => r.FoodRating.HasValue) ? (decimal?)Math.Round(src.Reviews.Where(r => r.FoodRating.HasValue).Average(r => r.FoodRating!.Value), 1) : null,
                src.Reviews != null && src.Reviews.Any(r => r.GuideRating.HasValue) ? (decimal?)Math.Round(src.Reviews.Where(r => r.GuideRating.HasValue).Average(r => r.GuideRating!.Value), 1) : null,
                src.Reviews != null && src.Reviews.Any(r => r.ValueRating.HasValue) ? (decimal?)Math.Round(src.Reviews.Where(r => r.ValueRating.HasValue).Average(r => r.ValueRating!.Value), 1) : null,
                ...
                (src.PackageMedia ?? Enumerable.Empty<PackageMedium>()).OrderBy(m => m.DisplayOrder).Select(m => new PackageMediaDto(m.Id, m.FileName, m.FilePath, m.MimeType, m.Category.ToString(), m.Caption, m.IsPrimary, m.DisplayOrder)).ToList(),
